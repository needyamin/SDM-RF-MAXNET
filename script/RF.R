...
hytune@models[[1]]
hytuneModel <- combineCV(hytune@models[[1]])
plotROC(hytuneModel, test = test)

# Predict with RF
predRF <- predict(hytune@models[[1]], data = bio, type = "cloglog")
...
om <- optimizeModel(hytune@models[[1]], hypers = h, metric = "auc", test = test, seed = 4)
...
ggsave("./result/RF/plotROC_optimize.jpg",dpi=300,width = 8,height = 5)

map <- predict(YAMiN, data = bio, type = "cloglog")
plotPred(map)
ggsave("./result/RF/map__optimizeModel.png",dpi=300,width = 8,height = 5)
...
# ggsave("./result/RF/map_Present_optimizeModel.png",dpi=300,width = 8,height = 5)
# 
# plotVarImp(varImp(YAMiN, permut = 5),color = "green")
# ggsave("./result/RF/plotVarImp_optimizeModel.png",dpi=300,width = 8,height = 5)
